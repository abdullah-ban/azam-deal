...
const Review = (props) => {
  ...
  const handleAddReview = async () => {
    ...
    var raw = JSON.stringify({
      ...
      pro_id: localStorage.getItem("Product ID"),
    ...
    await fetch(
      "https://team.flymingotech.in/azamDeals/public/api/review",
      requestOptions
    )
  ...
  const getAllReviews = async () => {
    ...
    await fetch(
      "https://team.flymingotech.in/azamDeals/public/api/getEnableReview",
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => {
  ...
  const GetCustomersReview = async () => {
    ...
    await fetch(
      "https://team.flymingotech.in/azamDeals/public/api/getByCidPid",
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => {
  ...
  const deleteMyReview = async (id) => {
    ...
    await fetch(
      `https://team.flymingotech.in/azamDeals/public/api/review/${id}`,
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => {
          ...
          <div className="mt-2 md:mt-5 lg:mt-5 w-full  pr-0 md:pr-10 lg:pr-10">
            ...
            <div className="p-2">
              <span className="ml-1 md:ml-4 lg:ml-5 font-semibold flex gap-2 items-center mt-2 underline text-[#008000] text-sm md:text-md lg:text-md">
                <GoPrimitiveDot /> Write about this product :)
              </span>
              <textarea
                cols="30"
                rows="3"
                className="mt-2 w-full appearance-none block bg-gray-200 text-gray-700 border border-gray-500 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-emerald-500"
                  ...
                  <div className="pb-3 md:pb-1 bg-white bg-opacity-40">
                    <div className="flex flex-wrap items-center">
                      <div className="flex items-center gap-4">
                        <p className="p-2 w-[45px] h-[40px] md:h-auto lg:h-auto rounded-full bg-[#008000] text-white">
                          <span className="ml-1 md:ml-2 lg:ml-2 font-bold text-xl uppercase">
                            {/* {el.Customer.name.slice(0, 1)} */}1
                          </span>
                        </p>
                        <h4 className="w-full md:w-auto text-xl font-heading font-medium">
                          {/* {el.Customer.name} */}name
                        </h4>
                      </div>
                      <div className="w-full md:w-px h-2 md:h-8 mx-8 bg-transparent md:bg-gray-200" />
                      <span className="mr-4 text-xl font-heading font-medium">
